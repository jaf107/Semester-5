...
public class MatrixChainMultiplicationRecursive {
	 ...
	 * 
	 * @param dim - stores the dimensions of matrices. dim[0] is the number of rows of the 1st matrix, dim[1] is the number of columns of the 1st matrix. 
	 * Then dim[1] is reused as the number of rows of the 2nd matrix, and dim[2] is the number of columns of the 2nd matrix. The dim[2] is then reused as the number of rows 
	 * of the 3rd matrix, and so on.  
	 * 
	 * @param i - left inclusive index to dims
	 * @param j - right inclusive index to dims
	 * @return
	 */
	static int cost(int dim[], int i, int j) { 
        if (i == j) return 0; 
  
        int min = Integer.MAX_VALUE; 
  
        for (int k = i; k < j; k++) { 
            int count = cost(dim, i, k) + cost(dim, k+1, j) + dim[i-1]*dim[k]*dim[j]; 
