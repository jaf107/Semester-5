...
class BPTree
{
    ...
    void insertInternal(dictionary, node *, node *);
...
dictionary BPTree::search(string x)
{
    dictionary d;
    d.bangla = "";
    d.english = "";
...
void BPTree::insert(dictionary key)
{
    if(root == NULL)
    {
...
void BPTree::insertInternal(dictionary key, node* cursor, node* child)
{
    //no overflow
    if(cursor->size < MAX)
    {
...
BPTree inputFromDict()
{
    BPTree tree;
    dictionary mykey;

    std::ifstream inputDic ("Translation.txt");
    for(std:: string line; getline(inputDic, line);)
    {
        string english, bangla;
        string tempString = line.substr(1);
        int pos = tempString.find("|");
        english = tempString.substr(0, pos);
        bangla = tempString.substr(pos+1);
...
void searchDict(BPTree tree)
{
    ofstream myDict;
    myDict.open("Search.txt");

    string toSearch;
    while(cin>>toSearch)
    {
        dictionary d = tree.search(toSearch);
        myDict << d.english << " " << d.bangla << endl;
    }
    myDict.close();
}
